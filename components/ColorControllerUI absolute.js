...
// Helper function for proportional scaling of two primary colors
// This ensures both colors reach their limits (0 or 255) at the same time
const scaleProportionally = (value1, value2, currentSecondary, newSecondary) => {
    const delta = newSecondary - currentSecondary;
    
    if (delta === 0) return { newValue1: Math.round(value1), newValue2: Math.round(value2) };
    
    if (delta > 0) {
        // Moving up - calculate how much each needs to travel to reach 255
        const remaining1 = 255 - value1;
        const remaining2 = 255 - value2;
        
        if (remaining1 <= 0 && remaining2 <= 0) {
            return { newValue1: Math.round(value1), newValue2: Math.round(value2) };
        }
        
        if (remaining1 <= 0) {
            // value1 is already at limit, only move value2
            return {
                newValue1: Math.round(value1),
                newValue2: Math.round(Math.min(255, value2 + delta))
            };
        }
        
        if (remaining2 <= 0) {
            // value2 is already at limit, only move value1
            return {
                newValue1: Math.round(Math.min(255, value1 + delta)),
                newValue2: Math.round(value2)
            };
        }
        
        // Both have room to grow - scale proportionally based on their distance to 255
        // The one further from 255 should move more to reach 255 at the same rate
        const maxRemaining = Math.max(remaining1, remaining2);
        
        // Calculate how much each should move based on their proportion of distance to limit
        const scale1 = remaining1 / maxRemaining;
        const scale2 = remaining2 / maxRemaining;
        
        const increase1 = delta * scale1;
        const increase2 = delta * scale2;
        
        return {
            newValue1: Math.round(Math.min(255, value1 + increase1)),
            newValue2: Math.round(Math.min(255, value2 + increase2))
        };
    } else {
        // Moving down - calculate how much each needs to travel to reach 0
        const available1 = value1;
        const available2 = value2;
        ...
        if (available1 <= 0) {
            ...
            return {
                newValue1: Math.round(value1),
                newValue2: Math.round(Math.max(0, value2 + delta))
            };
        }
        ...
        if (available2 <= 0) {
            // value2 is already at limit, only move value1
            return {
                newValue1: Math.round(Math.max(0, value1 + delta)),
                newValue2: Math.round(value2)
            };
        }
        
        // Both have room to decrease - scale proportionally based on their distance to 0
        // The one further from 0 should move more to reach 0 at the same rate
        const maxAvailable = Math.max(available1, available2);
        
        // Calculate how much each should move based on their proportion of distance to limit
        const scale1 = available1 / maxAvailable;
        const scale2 = available2 / maxAvailable;
        
        const decrease1 = Math.abs(delta) * scale1;
        const decrease2 = Math.abs(delta) * scale2;
        
        return {
            newValue1: Math.round(Math.max(0, value1 - decrease1)),
            newValue2: Math.round(Math.max(0, value2 - decrease2))
        };
    }
};
...
const ColorControllerUI = ({ row, col, width, height, hSpace, vSpace, bgColor = 'rgb(255,255,255)', initShapeColors, onPaletteChange, onBackgroundChange, onShapeColorsChange, hidePalette, showPalette }) => {
    ...
    // Track hover state for color picker button
   ...
   //const backgroundColor = 'rgb(255,255,255)'; // Initial background color
        ...
        if (isZPressed) {
            ...
            switch (color) {
                ...
                case 'Black':
                    ...
                    break;
                case 'Yellow':
                    // Use proportional scaling for Red and Green
                    const yellowResult = scaleProportionally(red, green, yellow, value);
                    newRed = yellowResult.newValue1;
                    newGreen = yellowResult.newValue2;
                    // Skip setYellow here to avoid circular dependency
                    // The useEffect that watches RGB will update yellow
                    break;
                case 'Cyan':
                    // Use proportional scaling for Green and Blue
                    const cyanResult = scaleProportionally(green, blue, cyan, value);
                    newGreen = cyanResult.newValue1;
                    newBlue = cyanResult.newValue2;
                    // Skip setCyan here to avoid circular dependency
                    // The useEffect that watches RGB will update cyan
                    break;
                case 'Magenta':
                    // Use proportional scaling for Red and Blue
                    const magentaResult = scaleProportionally(red, blue, magenta, value);
                    newRed = magentaResult.newValue1;
                    newBlue = magentaResult.newValue2;
    ...
    const handleShapeMouseDown = (index) => {
        // If shift is NOT pressed, always highlight only the single clicked shape
        if (!isShiftPressed) {
            setActiveShape(index);
            return;
        }
        
        // Shift IS pressed - handle group operations
        if (Array.isArray(selectedShape)) {
            // If we're adding to the group, include the new shape
            if (!selectedShape.includes(index)) {
                // Apply the current group's color to the newly added shape immediately
                ...
                // Highlight all current selected shapes plus the new one
                setActiveShape([...selectedShape, index]);
            } else {
                // Always activate all shapes in the selected group when pressing any of them
                setActiveShape([...selectedShape]);
            }
        } else if (selectedShape !== null && selectedShape !== 'background') {
            // If shift is pressed and we have a single shape selected, create group with both
            // Apply current color to the new shape
            const newShapeColors = [...shapeColors];
            newShapeColors[index] = outputColorCode; // Use current slider values
            setShapeColors(newShapeColors);
            ...
            // Fallback: just highlight the clicked shape
            setActiveShape(index);
        }
    };
    ...
    const handleShapeMouseUp = () => {
        // Always clear the active shape state on mouse up to restore 100% opacity
        setActiveShape(null);
    };
    ...
    return (
        <>
            ...
            {/* --------------- SLIDER PANEL ---------------- */}
            <div className="flex flex-col bg-background rounded-2xl absolute scale-75 bottom-5 right-0 lg:right-0 lg:scale-90 lg:bottom-9" 
                style={{ 
                    boxShadow: '2px 2px 5px rgba(0,0,0,0.3)',
                    position: 'absolute',
                    overflow: 'hidden',
            ...
            > 
                {/* Button area - full width, above gradient background, with mouse events */}
                <div 
                ...
                {/* Color Picker - 8 colored squares - shown on hover */}
                {isPickerHovered && (
                    <div 
                        onMouseEnter={() => setIsPickerHovered(true)}
                        onMouseLeave={() => setIsPickerHovered(false)}
                    ...
                    >
                        <div 
                            style={{ width: '32px', height: '30px', backgroundColor: 'rgb(255, 0, 0)', cursor: 'pointer', borderRadius: '0.5rem 0 0 0' }}
                            onClick={() => {
                                handleColorChange('rgb(255, 0, 0)');
                                setRed(255); setGreen(0); setBlue(0);
                                const cmy = rgbToCmy(255, 0, 0);
                                setCyan(cmy.c); setMagenta(cmy.m); setYellow(cmy.y);
                            }}
                        ></div>
                        <div 
                            style={{ width: '32px', height: '30px', backgroundColor: 'rgb(255, 128, 0)', cursor: 'pointer' }}
                            onClick={() => {
                                handleColorChange('rgb(255, 128, 0)');
                                setRed(255); setGreen(128); setBlue(0);
                                const cmy = rgbToCmy(255, 128, 0);
                                setCyan(cmy.c); setMagenta(cmy.m); setYellow(cmy.y);
                            }}
                        ></div>
                        <div 
                            style={{ width: '32px', height: '30px', backgroundColor: 'rgb(255, 255, 0)', cursor: 'pointer' }}
                            onClick={() => {
                                handleColorChange('rgb(255, 255, 0)');
                                setRed(255); setGreen(255); setBlue(0);
                                const cmy = rgbToCmy(255, 255, 0);
                                setCyan(cmy.c); setMagenta(cmy.m); setYellow(cmy.y);
                            }}
                        ></div>
                        <div 
                            style={{ width: '32px', height: '30px', backgroundColor: 'rgb(0, 255, 0)', cursor: 'pointer' }}
                            onClick={() => {
                                handleColorChange('rgb(0, 255, 0)');
                                setRed(0); setGreen(255); setBlue(0);
                                const cmy = rgbToCmy(0, 255, 0);
                                setCyan(cmy.c); setMagenta(cmy.m); setYellow(cmy.y);
                            }}
                        ></div>
                        <div 
                            style={{ width: '32px', height: '30px', backgroundColor: 'rgb(0, 255, 255)', cursor: 'pointer' }}
                            onClick={() => {
                                handleColorChange('rgb(0, 255, 255)');
                                setRed(0); setGreen(255); setBlue(255);
                                const cmy = rgbToCmy(0, 255, 255);
                                setCyan(cmy.c); setMagenta(cmy.m); setYellow(cmy.y);
                            }}
                        ></div>
                        <div 
                            style={{ width: '32px', height: '30px', backgroundColor: 'rgb(0, 0, 255)', cursor: 'pointer' }}
                            onClick={() => {
                                handleColorChange('rgb(0, 0, 255)');
                                setRed(0); setGreen(0); setBlue(255);
                                const cmy = rgbToCmy(0, 0, 255);
                                setCyan(cmy.c); setMagenta(cmy.m); setYellow(cmy.y);
                            }}
                        ></div>
                        <div 
                            style={{ width: '32px', height: '30px', backgroundColor: 'rgb(128, 0, 255)', cursor: 'pointer' }}
                            onClick={() => {
                                handleColorChange('rgb(128, 0, 255)');
                                setRed(128); setGreen(0); setBlue(255);
                                const cmy = rgbToCmy(128, 0, 255);
                                setCyan(cmy.c); setMagenta(cmy.m); setYellow(cmy.y);
                            }}
                        ></div>
                        <div 
                            style={{ width: '32px', height: '30px', backgroundColor: 'rgb(255, 0, 255)', cursor: 'pointer', borderRadius: '0 0.5rem 0 0' }}
                            onClick={() => {
                                handleColorChange('rgb(255, 0, 255)');
                                setRed(255); setGreen(0); setBlue(255);
                                const cmy = rgbToCmy(255, 0, 255);
                                setCyan(cmy.c); setMagenta(cmy.m); setYellow(cmy.y);
                            }}
                        ></div>
                    </div>
                ...
                {/* Picker icon - shown when not hovered */}
                {!isPickerHovered && (
                    <div 
                        style={{
                            position: 'absolute',
                ...
                {/* Spacer to maintain panel height */}
